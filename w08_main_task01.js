function main()
{
    ...
    for ( var i = 0; i < nfaces; i++ )
    {
        ...
        var S1 = scalars[ id[1] ];
        var S2 = scalars[ id[2] ];
        var C0 = SetColor(S0,S_min,S_max,cmap);
        var C1 = SetColor(S1,S_min,S_max,cmap);        
        var C2 = SetColor(S2,S_min,S_max,cmap);
    ...
    function SetColor(S,S_min,S_max,cmap)
    {
	var length = cmap.length;
	var id = ((S-S_min)/(S_max-S_min))*(length-1);
	var id0 = Math.floor(id);
	var id1 = Math.min(id0+1,length-1);
	var t = id -id0;
	var C0 = new THREE.Color().setHex(cmap[id0][1]);
	var C1 = new THREE.Color().setHex(cmap[id1][1]);
